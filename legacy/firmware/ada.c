...
static bool layoutToken(const CardanoToken *msg, bool is_mint) {
  ...
  if (is_mint) {
    layoutDialogSwipeEx(&bmp_icon_question, _("Cancel"), _("Confirm"),
                        tx_msg[0], _("Asset Fingerprint:"), fingerprint, NULL,
                        NULL, NULL, NULL, FONT_FIXED);
  } else {
    layoutDialogSwipe(&bmp_icon_question, _("Cancel"), _("Confirm"), NULL, NULL,
  ...
  if (is_mint) {
    layoutDialogSwipeEx(&bmp_icon_question, _("Cancel"), _("Confirm"),
                        tx_msg[0], _("Token Amount:"), amount, NULL, NULL, NULL,
                        NULL, FONT_FIXED);
  } else {
    layoutDialogSwipe(&bmp_icon_question, _("Cancel"), _("Confirm"), NULL, NULL,
...
static bool layoutCertificate(const CardanoTxCertificate *cert) {
  ...
  layoutDialogSwipe(&bmp_icon_question, _("Cancel"), _("Confirm"), NULL,
                    _("Transaction Type:"), tx_type, NULL, NULL, NULL, NULL);
  if (!protectButton(ButtonRequestType_ButtonRequest_SignTx, false)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
    return false;
  }
...
bool ada_sign_messages(const CardanoSignMessage *msg,
  ...
  layoutVerifyAddress(NULL, address_str);
