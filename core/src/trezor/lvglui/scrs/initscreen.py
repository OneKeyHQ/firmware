...
class InitScreen(Screen):
    ...
    class QuickStart(Screen):
        def __init__(self, prev_scr):
            ...
            self.subtitle.set_text("Create a new wallet, or restore wallet used before from a backup.")
...
class CreateNew(Screen):
    ...
    class BackupTip(Screen):
        def __init__(self, prev_scr):
            ...
            self.subtitle = lv.label(self)
            self.subtitle.set_long_mode(lv.label.LONG.WRAP)
            self.subtitle.set_text("Next, OneKey will display a list of words, which is called the recovery phrase of wallet. So you need to know:")
            self.subtitle.set_width(lv.pct(80))
            self.subtitle.set_height(lv.SIZE.CONTENT)   # 1
            self.subtitle.set_x(0)
        ...
        class ManualBackup(Screen):
            def __init__(self, prev_scr):
                ...
                self.subtitle.set_long_mode(lv.label.LONG.WRAP)
                global select_word_cnt
                self.subtitle.set_text(f"Write down the following {select_word_cnt} words in order.")
                self.subtitle.set_width(lv.pct(80))
                self.subtitle.set_height(lv.SIZE.CONTENT)   # 1
                self.subtitle.set_x(0)
            ...
            class CheckRecoveryPhrase(Screen):
                def __init__(self, prev_scr):
                    ...
                    self.subtitle.set_long_mode(lv.label.LONG.WRAP)
                    global select_word_cnt
                    self.subtitle.set_text(f"Check {select_word_cnt} words again, make sure they are exactly the same as the backup you just wrote down.")
                    self.subtitle.set_width(lv.pct(80))
                    self.subtitle.set_height(lv.SIZE.CONTENT)   # 1
                    self.subtitle.set_x(0)
                    self.subtitle.set_y(lv.pct(30))
                ...
                class CheckWord(Screen):
                    ...
                    def on_click(self, target):
                        ...
                        if check_selected == mnemonics[check_index]:
                            ...
                            self.subtitle.set_style_text_color(lv_colors.RED, lv.PART.MAIN | lv.STATE.DEFAULT)
...
class RecoveryFail(Screen):
    ...
    def __init__(self, subtitle="The recovery phrase you entered is invalid. Check your backup carefully and try again."):
        if not hasattr(self, "_init"):
                self._init = True
        else:
            return
