...
Onekey has limited support for logical sessions. The main purpose is to enable seamless
...
After Onekey starts up, no session exists. Any attempt to use session data (i.e., the
...
`EndSession` call. The session and all its associated data is wiped from Onekey memory,
and it is impossible to resume the session. Onekey returns to the initial state and
...
Sessions only exist in RAM and are lost when Onekey is disconnected.

All commands are performed in the context of the current session, until one of the
following happens:

* Host calls `EndSession`. The current session is destroyed and Onekey returns to the
  initial state.
